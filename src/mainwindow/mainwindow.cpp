...
void MainWindow::on_LightEngineConnectButton_clicked() {
    ...
    LightEngineSelect = (LightEngineSelect_t) ui->LightEngineDropdown->currentIndex();

    LightEngineSelect_f(LightEngineSelect);

    switch(LightEngineSelect)
    {

    case LIGHTENGINE_DLI:
    case LIGHTENGINE_FIREBIRD:
    ...
    case LIGHTENGINE_DEBUG:

        PrintToTerminal("Debug Light Engine \"Connected\"");
        ui->LightEngineIndicator->setStyleSheet("background:rgb(0, 255, 0); border: 1px solid black;");
        ui->LightEngineIndicator->setText("Connected");
        break;
    }
...
void MainWindow::on_StageConnectButton_clicked() {
    StageSelect = (StageSelect_t) ui->StageDropdown->currentIndex();

    StageSelect_f(StageSelect);

    switch(StageSelect)

    {
    case STAGE_DEBUG:
        PrintToTerminal("Debug Stage \"Connected\"");
...
void MainWindow::on_RollToRollDropdown_activated(int index)
{
    RollToRollSelect = (RollToRollSelect_t) index;
    RollToRollSelect_f(RollToRollSelect);

    PrintToTerminal("Selected Roll To Roll: " + ui->RollToRollDropdown->currentText());
}

void MainWindow::on_StageDropdown_activated(int index)
{
    StageSelect = (StageSelect_t) index;
    StageSelect_f(StageSelect);

    PrintToTerminal("Selected Stage: " + ui->StageDropdown->currentText());
}

void MainWindow::on_LightEngineDropdown_activated(int index)
{
    LightEngineSelect = (LightEngineSelect_t) index;
    LightEngineSelect_f(LightEngineSelect);

    PrintToTerminal("Selected Light Engine: " + ui->LightEngineDropdown->currentText());
}

void MainWindow::on_ProjectionModeDropdown_activated(int index)
{
    ProjectionModeSelect = (ProjectionModeSelect_t) index;
    ProjectionModeSelect_f(ProjectionModeSelect);

    PrintToTerminal("Selected Projection Mode: " + ui->ProjectionModeDropdown->currentText());
}



void MainWindow::PumpSelect_f(PumpSelect_t index)
{
    switch(index)
    {
    case PUMP_NONE:
...
void MainWindow::RollToRollSelect_f(RollToRollSelect_t index)
{

}

void MainWindow::StageSelect_f(StageSelect_t index)
{
    switch(index)
    {
    case STAGE_GGCODE:
        ...
        // Disable the starting position selection and stage software limits
        ui->SettingsWidget->EnableParameter(STARTING_POSITION, OFF);
        ui->SettingsWidget->EnableParameter(MAX_END, OFF);
        ui->SettingsWidget->EnableParameter(MIN_END, OFF);
    ...
    case STAGE_SMC100CC:
        ...
        // Enable the starting position selection and stage software limits
        ui->SettingsWidget->EnableParameter(STARTING_POSITION, ON);
        ui->SettingsWidget->EnableParameter(MAX_END, ON);
        ui->SettingsWidget->EnableParameter(MIN_END, ON);
    ...
    case STAGE_KVS30:
        m_PrintSettings.StageType = STAGE_KVS;  // Set stage to KVS30/M
...
void MainWindow::LightEngineSelect_f(LightEngineSelect_t index)
{
    switch(index)
    {
    case LIGHTENGINE_DLI:
...
void MainWindow::ProjectionModeSelect_f(ProjectionModeSelect_t index)
{
    switch (index)
    {
    case PROJECTIONMODE_POTF:
        // Enable/disable relevant parameters
        ui->SettingsWidget->EnableParameter(MAX_IMAGE, ON);
        ui->SettingsWidget->EnableParameter(VP_RESYNC, OFF);
        ui->SettingsWidget->EnableParameter(INITIAL_DELAY, OFF);
